...
//extern keyword in tcp_client.h
char detected_keyword[MAX_ARRAY_LENGTH];
...
void cyberon_asr_process(short *lpsSample, int nNumSample)
{
	...
	if((nErr = DSpotter_AddSample(g_hDSpotter, lpsSample, nNumSample)) == DSPOTTER_SUCCESS)
	{
		...
		if(strncmp(pchCommand, "你好", strlen("你好"))){
			memset(detected_keyword, '\0', MAX_ARRAY_LENGTH);
			strncpy(detected_keyword, pchCommand, strlen(pchCommand));
			int msg = 0;
			xQueueSend(qh, &msg, pdMS_TO_TICKS(500));
			vTaskDelay(1000);
			vTaskSuspend(NULL);
		}
	}
